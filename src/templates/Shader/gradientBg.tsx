...
const GradientBg = (props: Mesh) => {
  const { scale, lightness } = useControls({
    scale: { value: 2.37, min: 1.5, max: 3 },
    lightness: { value: 0.3, min: -1, max: 1 },
  });
  ...
  const advanced = useControls({
    advanced: folder({
      ...
      strength: { value: 1.52, min: 0.2, max: 3 },
    }, { collapsed: true })
  });
  ...
  return (
    ...
    >
      ...
      {/* @ts-ignore */}
      <waterGradientMaterial key={WaterGradientMaterial.key} ref={materialRef} uLightness={lightness} uSpeed={animation.speed * 0.01} uNoiseDensity={advanced.density} uNoiseStrength={advanced.strength} />
