...
import vertex from "./multiStopGradient.vert";
import fragment from "./multiStopGradient.frag";

// custom shader material
const MultiStopGradientMaterial = shaderMaterial(
    {
        ...
        uComplex: 1.0,
        uMorph: 0.0,
        uColor: ["#C4C4C4", "#E48D3E", "#DF2512", "#1F1013", "#000000"].map(
...
// @ts-ignore
MultiStopGradientMaterial.key = THREE.MathUtils.generateUUID();

extend({ MultiStopGradientMaterial });

// shader material combined with mesh
const MultiStopGradient: React.FC = (props: Mesh) => {
    ...
    return (
        ...
        >
            ...
            {/* @ts-ignore */}
            <multiStopGradientMaterial
                key={MultiStopGradientMaterial.key}
                ref={materialRef}
                uSpeed={speed}
                ...
                uComplex={complex}
                uMorph={morph}
                uColor={[color1, color2, color3, color4, bgColor].map((color) => new THREE.Color(color))}
