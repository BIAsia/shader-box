...
void main() {
    ...
    for(int i = 0; i < 4; i++) {
        ...
        float scale = mix(0.2, 1.4, layerTime);
        float fade = 1.0 - smoothstep(0.2, 1.3, scale);
        ...
        float rotateTime = mod(time, 100.0);
        if(i == 0) {
            vec3 colorArr[4];
            colorArr[0] = uColor[1];
            colorArr[1] = uColor[3];
            colorArr[2] = uColor[2];
            colorArr[3] = uColor[1];
            grad = getGradientColor(position * .1, 0.5 * rotateTime, fi * 2.1, colorArr, uBgColor);
        } else if(i == 1) {
            vec3 colorArr[4];
            colorArr[0] = uColor[0];
            colorArr[1] = uColor[2];
